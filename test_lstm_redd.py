...
def train_lstm_on_appliance(data_dict, appliance_name, window_size=100,
                             hidden_size=128, num_layers=2, bidirectional=True,
                             epochs=80, lr=0.001, patience=20,
                             save_dir='models/lstm_redd'):
    ...
    model = LSTMModel(
        ...
        output_size=1, bidirectional=bidirectional
    ).to(device)
    ...
    print(f"Starting LSTM training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"lstm_redd_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"lstm_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'lstm_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_lstm_on_all_redd_appliances(window_size=100, hidden_size=128, num_layers=2,
                                     bidirectional=True, epochs=80, lr=0.001, patience=20):

    data_dict = load_redd_splits()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_save_dir = f"models/lstm_redd_{timestamp}"
    ...
    for appliance_name in appliances:
        print(f"\n{'='*60}")
        print(f"Testing LSTM on {appliance_name}")
        print(f"{'='*60}\n")
        ...
        try:
            model, history, test_metrics = train_lstm_on_appliance(
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"lstm_redd_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    summary = {
        'timestamp': timestamp, 'dataset': 'REDD', 'model': 'LSTM',
        'window_size': window_size,
    ...
    print(f"\nLSTM REDD testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing LSTM on REDD dataset...")
    ...
    test_lstm_on_all_redd_appliances(
        window_size=100, hidden_size=128, num_layers=2,
        bidirectional=True, epochs=80, lr=0.001, patience=20)
