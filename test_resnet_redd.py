...
def train_resnet_on_appliance(data_dict, appliance_name, window_size=100,
    ...
    X_train, _ = create_sequences(train_data, window_size=window_size, target_size=1)
    X_val,   _ = create_sequences(val_data,   window_size=window_size, target_size=1)
    X_test,  _ = create_sequences(test_data,  window_size=window_size, target_size=1)
    ...
    model = ResNetModel(
        input_size=1, output_size=1, layers=layers, base_width=base_width
    ).to(device)
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"resnet_redd_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"resnet_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'resnet_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_resnet_on_all_redd_appliances(window_size=100, base_width=32, layers=None,
                                       epochs=80, lr=0.001, patience=20):
    if layers is None:
        layers = [2, 2, 2]

    data_dict = load_redd_splits()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_save_dir = f"models/resnet_redd_{timestamp}"
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"resnet_redd_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    summary = {
        'timestamp': timestamp, 'dataset': 'REDD', 'model': 'ResNet',
        'window_size': window_size,
    ...
    for appliance, result in all_results.items():
        ...
        print(f"  {appliance:<15}  F1={m['f1']:.4f}  P={m['precision']:.4f}  "
...
if __name__ == "__main__":
    ...
    for f in ['data/redd/train_small.pkl', 'data/redd/val_small.pkl', 'data/redd/test_small.pkl']:
        if not os.path.exists(f):
            print(f"Error: {f} not found!"); import sys; sys.exit(1)

    test_resnet_on_all_redd_appliances(
