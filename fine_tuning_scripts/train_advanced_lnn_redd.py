"""
Advanced LNN (multi-layer adaptive LTC) — REDD Dataset with Fine-tuning

Uses AdvancedLiquidNetworkModel: stacked AdvancedLiquidTimeLayer units with
input-dependent adaptive tau modulation and gating.
...
def train_on_appliance(splits, redd_appliance, data_dir=DEFAULT_DATA_DIR,
                       ...
                       save_dir='models/advanced_lnn_redd_finetune'):
    os.makedirs(save_dir, exist_ok=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\nAppliance: {redd_appliance}  |  device: {device}  "
          f"hidden={hidden_size}  layers={num_layers}  dt={dt}")
    ...
    model = AdvancedLiquidNetworkModel(
        ...
        num_layers=num_layers, dt=dt).to(device)
    ...
    plt.savefig(os.path.join(save_dir,
                    f'advanced_lnn_redd_{redd_appliance.replace(" ", "_")}_metrics.png'),
    ...
    with open(os.path.join(save_dir,
                  f'advanced_lnn_redd_{redd_appliance.replace(" ", "_")}_results.json'),
...
def run_all(data_dir=DEFAULT_DATA_DIR, hidden_size=64, num_layers=2, dt=0.1):
    ...
    base_dir  = os.path.join(
        ...
        f'models/advanced_lnn_redd_finetune_{timestamp}')
    all_results = {}
    wall_start  = time.time()

    for redd_app in REDD_APPLIANCES:
        print(f"\n{'='*60}\nAdvanced LNN REDD — {redd_app}\n{'='*60}")
    ...
    print(f"\nAdvanced LNN REDD complete.  Results → {base_dir}")
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='Advanced LNN REDD fine-tuning')
    p.add_argument('--data-dir',    default=DEFAULT_DATA_DIR)
    p.add_argument('--hidden-size', type=int,   default=64)
    p.add_argument('--num-layers',  type=int,   default=2)
    p.add_argument('--dt',          type=float, default=0.1)
    args = p.parse_args()
    run_all(args.data_dir, args.hidden_size, args.num_layers, args.dt)
