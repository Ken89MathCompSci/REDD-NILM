...
class MultiTimescaleLiquidLayer(nn.Module):
    """
    Advanced LNN v5 — two coupled LNN streams (fast + slow) sharing combined context.

    Input:  xe (B, input_size)  where xe = cat([x_t, e_t])
    State:  h_fast (B, half),  h_slow (B, half)
    Context: ctx = cat([xe, h_fast, h_slow])

    Each stream computes an input-dependent gate and time constant, then integrates
    via Euler: h ← h + dt * (-h/τ + gate * tanh(proj(xe) + rec(h)))
    """
...
class MultiTimescaleLNNModel(nn.Module):
    """
    Advanced LNN v5 — multi-timescale + event-aware tau + attentive pooling.

    x (B, T, 1)
    → e_t = |x_t - x_{t-1}|        event signal
    → xe  = cat([x, e])             (B, T, 2)
    → MultiTimescaleLiquidLayer per timestep → states (B, T, hidden)
    → scalar softmax attention → context (B, hidden)
    → fc → output (B, 1)
    """
