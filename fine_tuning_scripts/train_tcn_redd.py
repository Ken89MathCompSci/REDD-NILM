"""
TCN (Temporal Convolutional Network) — REDD Dataset with Fine-tuning
...
def train_on_appliance(splits, redd_appliance, data_dir=DEFAULT_DATA_DIR,
                       ...
                       save_dir='models/tcn_redd_finetune'):
    ...
    plt.savefig(os.path.join(save_dir,
                    f'tcn_redd_{redd_appliance.replace(" ", "_")}_metrics.png'),
    ...
    with open(os.path.join(save_dir,
                  f'tcn_redd_{redd_appliance.replace(" ", "_")}_results.json'),
...
def run_all(data_dir=DEFAULT_DATA_DIR, num_channels=None, kernel_size=3, dropout=0.2):
    if num_channels is None:
        num_channels = [32, 32, 64]
    ...
    base_dir  = os.path.join(
        ...
        f'models/tcn_redd_finetune_{timestamp}')
    all_results = {}
    ...
    print(f"\nTCN REDD complete.  Results → {base_dir}")
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='TCN REDD fine-tuning')
    p.add_argument('--data-dir',    default=DEFAULT_DATA_DIR)
    p.add_argument('--kernel-size', type=int,   default=3)
    p.add_argument('--dropout',     type=float, default=0.2)
    args = p.parse_args()
    run_all(args.data_dir, kernel_size=args.kernel_size, dropout=args.dropout)
