"""
ResNet — REDD Dataset with Fine-tuning
...
def train_on_appliance(splits, redd_appliance, data_dir=DEFAULT_DATA_DIR,
                       layers=None, base_width=32,
                       save_dir='models/resnet_redd_finetune'):
    if layers is None:
        layers = [2, 2, 2]
    os.makedirs(save_dir, exist_ok=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\nAppliance: {redd_appliance}  |  device: {device}  "
          f"layers={layers}  base_width={base_width}")
    ...
    model = ResNetModel(
        input_size=1, output_size=1, layers=layers,
        base_width=base_width).to(device)
    ...
    plt.savefig(os.path.join(save_dir,
                    f'resnet_redd_{redd_appliance.replace(" ", "_")}_metrics.png'),
    ...
    config = {
        ...
        'model_params': {'layers': layers, 'base_width': base_width},
    ...
    with open(os.path.join(save_dir,
                  f'resnet_redd_{redd_appliance.replace(" ", "_")}_results.json'),
...
def run_all(data_dir=DEFAULT_DATA_DIR, layers=None, base_width=32):
    if layers is None:
        layers = [2, 2, 2]
    ...
    print(f"\nResNet REDD complete.  Results → {base_dir}")
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='ResNet REDD fine-tuning')
    p.add_argument('--data-dir',    default=DEFAULT_DATA_DIR)
    p.add_argument('--base-width',  type=int, default=32)
    args = p.parse_args()
    run_all(args.data_dir, base_width=args.base_width)
