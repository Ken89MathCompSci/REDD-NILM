"""
GRU — REDD Dataset with Fine-tuning
...
def train_on_appliance(splits, redd_appliance, data_dir=DEFAULT_DATA_DIR,
                       ...
                       save_dir='models/gru_redd_finetune'):
    ...
    plt.savefig(os.path.join(save_dir,
                    f'gru_redd_{redd_appliance.replace(" ", "_")}_metrics.png'),
    ...
    with open(os.path.join(save_dir,
                  f'gru_redd_{redd_appliance.replace(" ", "_")}_results.json'),
...
def run_all(data_dir=DEFAULT_DATA_DIR, hidden_size=64, num_layers=2, dropout=0.0):
    ...
    base_dir  = os.path.join(
        ...
        f'models/gru_redd_finetune_{timestamp}')
    all_results = {}
    ...
    print(f"\nGRU REDD complete.  Results → {base_dir}")
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='GRU REDD fine-tuning')
