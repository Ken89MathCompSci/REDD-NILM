"""
Advanced LNN v5 — REDD Dataset with Fine-tuning

Combines the multi-timescale event-aware LNN architecture (v5) with a 3-phase
fine-tuning pipeline applied to REDD data.

Architecture (v5):
  - Fast + slow LNN streams with shared context
  - Event signal e_t = |x_t - x_{t-1}| fed as additional input
  - Appliance-specific tau ranges
  - Scalar softmax attention pooling over all T hidden states

3-phase pipeline:
  Phase 1 — Pretrain:  data/redd/train_small.pkl    (general training data)
  Validation:          data/redd/val_small.pkl       (early stopping criterion)
  Phase 2 — Fine-tune: data/redd/test_small.pkl[0]  (target house, first split)
...
from models import MultiTimescaleLiquidLayer, MultiTimescaleLNNModel
...
# REDD column names for each appliance
REDD_APPLIANCES = ['dish washer', 'fridge', 'microwave', 'washer dryer']

# Map REDD column name → canonical key for tau lookup
REDD_TO_CANONICAL = {
...
# Appliance-specific tau ranges derived from v3 diagnostics
APPLIANCE_TAU = {
...
def load_redd_splits(data_dir=DEFAULT_DATA_DIR):
    ...
    # Fine-tune: first house in test set (different domain from pretrain)
    ft_data = test_list[0]

    # Test: House 3, 2011-04-30 (consistent with all other REDD scripts)
    test_df   = test_list[3]
    test_data = test_df[
...
def create_sequences(data, appliance_col):
    """Sliding-window sequence-to-point with midpoint labelling."""
...
def train_on_appliance(splits, redd_appliance, data_dir=DEFAULT_DATA_DIR,
                       ...
                       save_dir='models/advanced_lnn_v5_redd_finetune'):
    os.makedirs(save_dir, exist_ok=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    canonical  = REDD_TO_CANONICAL[redd_appliance]
    tau_cfg    = APPLIANCE_TAU[canonical]
    tau_fast   = tau_cfg['fast']
    tau_slow   = tau_cfg['slow']
    print(f"\nAppliance: {redd_appliance} (canonical={canonical})  |  device: {device}  "
        ...
        if va_loss < best_val:
            ...
            save_model(model,
                       ...
                       m, os.path.join(save_dir,
                           f'pretrain_{canonical}_best.pth'))
        else:
            counter += 1
            if counter >= PATIENCE:
    ...
    plt.title(f'Pretrain Loss — {canonical}'); plt.xlabel('Epoch')
    ...
    plt.title(f'MAE — {canonical}'); plt.xlabel('Epoch'); plt.legend(); plt.grid(alpha=0.3)
    plt.subplot(2, 3, 3)
    ...
    plt.title(f'SAE — {canonical}'); plt.xlabel('Epoch'); plt.legend(); plt.grid(alpha=0.3)
    plt.subplot(2, 3, 4)
    ...
    plt.title(f'F1 — {canonical}'); plt.xlabel('Epoch'); plt.legend(); plt.grid(alpha=0.3)
    plt.subplot(2, 3, 5)
    ft_ep = range(1, len(ft_history['train_loss']) + 1)
    plt.plot(ft_ep, ft_history['train_loss'], color='green', label='FT train loss')
    plt.title(f'Fine-tune Loss — {canonical}'); plt.xlabel('FT Epoch')
    plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir,
                    f'advanced_lnn_v5_redd_{canonical}_metrics.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    # -- JSON -----------------------------------------------------------------
    config = {
        'appliance_redd': redd_appliance,
        'appliance':      canonical,
        'dataset': 'REDD',
    ...
    with open(os.path.join(save_dir,
                  f'advanced_lnn_v5_redd_{canonical}_results.json'),
...
def run_all(data_dir=DEFAULT_DATA_DIR, hidden_size=64, dt=0.1):
    ...
    base_dir  = os.path.join(
        ...
        f'models/advanced_lnn_v5_redd_finetune_{timestamp}')
    all_results = {}
    wall_start  = time.time()

    for redd_app in REDD_APPLIANCES:
        canonical = REDD_TO_CANONICAL[redd_app]
        print(f"\n{'='*60}\nAdvanced LNN v5 REDD — {redd_app}\n{'='*60}")
        app_dir = os.path.join(base_dir, canonical)
        try:
    ...
    for canonical, r in all_results.items():
        tau = APPLIANCE_TAU[canonical]
        print(f"  {canonical:<20}  F1  "
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(
        description='Advanced LNN v5 REDD fine-tuning: '
                    'multi-timescale + event-aware tau + attention')
    p.add_argument('--data-dir',    default=DEFAULT_DATA_DIR,
                   help='Path to directory containing REDD pkl files')
