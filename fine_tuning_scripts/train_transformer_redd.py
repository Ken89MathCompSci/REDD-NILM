"""
Transformer — REDD Dataset with Fine-tuning
...
def train_on_appliance(splits, redd_appliance, data_dir=DEFAULT_DATA_DIR,
                       hidden_size=64, num_layers=3, num_heads=4, dropout=0.1,
                       save_dir='models/transformer_redd_finetune'):
    ...
    plt.savefig(os.path.join(save_dir,
                    f'transformer_redd_{redd_appliance.replace(" ", "_")}_metrics.png'),
    ...
    with open(os.path.join(save_dir,
                  f'transformer_redd_{redd_appliance.replace(" ", "_")}_results.json'),
...
def run_all(data_dir=DEFAULT_DATA_DIR, hidden_size=64, num_layers=3,
    ...
    base_dir  = os.path.join(
        ...
        f'models/transformer_redd_finetune_{timestamp}')
    all_results = {}
    wall_start  = time.time()

    for redd_app in REDD_APPLIANCES:
        print(f"\n{'='*60}\nTransformer REDD — {redd_app}\n{'='*60}")
        ...
        try:
            _, _, after, before = train_on_appliance(
                ...
                num_heads=num_heads, dropout=dropout, save_dir=app_dir)
    ...
    print(f"\nTransformer REDD complete.  Results → {base_dir}")
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='Transformer REDD fine-tuning')
    ...
    args = p.parse_args()
    run_all(args.data_dir, args.hidden_size, args.num_layers,
