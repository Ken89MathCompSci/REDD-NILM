"""
LSTM — REDD Dataset with Fine-tuning
...
def train_on_appliance(splits, redd_appliance, data_dir=DEFAULT_DATA_DIR,
                       ...
                       save_dir='models/lstm_redd_finetune'):
    ...
    plt.savefig(os.path.join(save_dir,
                    f'lstm_redd_{redd_appliance.replace(" ", "_")}_metrics.png'),
    ...
    with open(os.path.join(save_dir,
                  f'lstm_redd_{redd_appliance.replace(" ", "_")}_results.json'),
...
def run_all(data_dir=DEFAULT_DATA_DIR, hidden_size=64, num_layers=2, dropout=0.0):
    ...
    base_dir  = os.path.join(
        ...
        f'models/lstm_redd_finetune_{timestamp}')
    all_results = {}
    ...
    print(f"\nLSTM REDD complete.  Results → {base_dir}")
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='LSTM REDD fine-tuning')
    p.add_argument('--data-dir',    default=DEFAULT_DATA_DIR)
    p.add_argument('--hidden-size', type=int,   default=64)
    p.add_argument('--num-layers',  type=int,   default=2)
    p.add_argument('--dropout',     type=float, default=0.0)
    args = p.parse_args()
    run_all(args.data_dir, args.hidden_size, args.num_layers, args.dropout)
