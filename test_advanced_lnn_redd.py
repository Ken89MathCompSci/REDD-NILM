...
def train_on_appliance(data_dict, appliance_name, window_size=100,
                       ...
                       save_dir='models/advanced_lnn_redd'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"advanced_lnn_redd_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"advanced_lnn_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'advanced_lnn_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_on_all_appliances(window_size=100, hidden_size=64, num_layers=2, dt=0.1,
    ...
    base_save_dir = f"models/advanced_lnn_redd_{timestamp}"
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"advanced_lnn_redd_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    summary = {
        'timestamp': timestamp, 'dataset': 'REDD', 'model': 'AdvancedLiquidNetworkModel',
        'window_size': window_size,
    ...
    print(f"\nAdvanced-LNN REDD testing completed. Results saved to {base_save_dir}")
    for app in appliances:
        if app in all_results:
            m = all_results[app]['final_metrics']
            print(f"  {app:<15}  F1={m['f1']:.4f}  P={m['precision']:.4f}  "
                  f"R={m['recall']:.4f}  MAE={m['mae']:.2f}  SAE={m['sae']:.4f}")
    return all_results


if __name__ == "__main__":
    print("Testing Advanced-LNN on REDD dataset...")
    ...
    test_on_all_appliances(
        window_size=100, hidden_size=64, num_layers=2, dt=0.1,
        epochs=80, lr=0.001, patience=20)
