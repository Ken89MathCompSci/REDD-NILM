"""
GRU baseline for NILM -- REDD CSV splits.

Companion to APR-scripts-UK-dale/test_gru_apr_new_house2_dataset.py, adapted
...
Architecture/training loop unchanged: one GRU model per appliance
...
def train_gru_on_appliance(data_dict, appliance_name, window_size=100,
                            ...
                            save_dir='models/gru_redd_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"gru_redd_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"gru_redd_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'gru_redd_{appliance_name}_history.json'),
...
def test_gru_on_all_appliances(window_size=100, hidden_size=128, num_layers=2,
                                bidirectional=True, epochs=80, lr=0.001, patience=20):

    data_dict = load_data()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"gru_redd_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"gru_redd_{appliance_name}_best.pth"),
                    'final_metrics': {k: float(v) for k, v in test_metrics.items()}
                }
                print(f"Successfully tested GRU on {appliance_name}")
...
if __name__ == "__main__":
    ...
    results = test_gru_on_all_appliances(
