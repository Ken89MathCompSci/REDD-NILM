"""
ResNet (1D) baseline for NILM -- REDD h5 splits.

Companion to APR-scripts-UK-dale/test_resnet_apr_new_house2_dataset.py,
...
Architecture/training loop unchanged: one ResNetModel per appliance
(sequence-to-point, single output), MSE loss, Adam, ReduceLROnPlateau,
early stopping on val loss, best-checkpoint restore before test evaluation.
"""
...
def train_resnet_on_appliance(data_dict, appliance_name, window_size=100,
                               base_width=32, layers=None,
                               epochs=80, lr=0.001, patience=20,
                               save_dir='models/resnet_redd_dataset'):
    if layers is None:
        layers = [2, 2, 2]
    ...
    print(f"Starting ResNet training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"resnet_redd_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_redd_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'resnet_redd_{appliance_name}_history.json'),
...
def test_resnet_on_all_appliances(window_size=100, base_width=32, layers=None,
                                   epochs=80, lr=0.001, patience=20):
    if layers is None:
        layers = [2, 2, 2]
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"resnet_redd_dataset_{timestamp}")

    all_results = {}

    for appliance_name in APPLIANCES:
        print(f"\n{'='*60}")
        print(f"Testing ResNet on {appliance_name}")
        print(f"{'='*60}\n")
        ...
        try:
            model, history, test_metrics = train_resnet_on_appliance(
                data_dict, appliance_name=appliance_name,
                window_size=window_size, base_width=base_width, layers=layers,
                epochs=epochs, lr=lr, patience=patience, save_dir=appliance_dir)
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"resnet_redd_{appliance_name}_best.pth"),
                    'final_metrics': {k: float(v) for k, v in test_metrics.items()}
                }
                print(f"Successfully tested ResNet on {appliance_name}")
    ...
    summary = {
        ...
        'window_size': window_size,
        'model_params': {'base_width': base_width, 'layers': layers},
    ...
    print(f"\nResNet REDD testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing ResNet on REDD dataset...")
    ...
    results = test_resnet_on_all_appliances(
        window_size=100, base_width=32, layers=[2, 2, 2],
        epochs=80, lr=0.001, patience=20)

    print(f"\nSummary of ResNet testing on REDD dataset:")
