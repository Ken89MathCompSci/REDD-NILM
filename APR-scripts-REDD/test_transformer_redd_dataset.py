"""
Simplified Transformer baseline for NILM -- REDD h5 splits.

Companion to APR-scripts-UK-dale/test_transformer_apr_new_house2_dataset.py,
...
Architecture/training loop unchanged: one SimpleTransformerModel per
appliance (sequence-to-point, single output), MSE loss, Adam,
ReduceLROnPlateau, early stopping on val loss, best-checkpoint restore
...
def train_transformer_on_appliance(data_dict, appliance_name, window_size=100,
                                    hidden_size=128, num_layers=3, num_heads=4, dropout=0.1,
                                    epochs=80, lr=0.001, patience=20,
                                    save_dir='models/transformer_redd_dataset'):
    ...
    print(f"Starting Transformer training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"transformer_redd_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"transformer_redd_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'transformer_redd_{appliance_name}_history.json'),
...
def test_transformer_on_all_appliances(window_size=100, hidden_size=128, num_layers=3,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"transformer_redd_dataset_{timestamp}")

    all_results = {}

    for appliance_name in APPLIANCES:
        print(f"\n{'='*60}")
        print(f"Testing Transformer on {appliance_name}")
        print(f"{'='*60}\n")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"transformer_redd_{appliance_name}_best.pth"),
                    'final_metrics': {k: float(v) for k, v in test_metrics.items()}
                }
                print(f"Successfully tested Transformer on {appliance_name}")
    ...
    print(f"\nTransformer REDD testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Transformer on REDD dataset...")
    ...
    results = test_transformer_on_all_appliances(
        ...
        dropout=0.1, epochs=80, lr=0.001, patience=20)

    print(f"\nSummary of Transformer testing on REDD dataset:")
