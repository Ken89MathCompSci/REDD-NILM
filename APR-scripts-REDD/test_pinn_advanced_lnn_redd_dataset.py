...
Architecture, loss, and training loop are otherwise unchanged from the
UK-dale companion script (this is the "advanced" upgrade over the
single-layer PINN-LNN: a 2-layer stacked AdvancedLiquidNetworkModel encoder
with inter-layer LayerNorm, vs. the single AdvancedLiquidTimeLayer used in
test_pinn_lnn_redd_dataset.py):
...
Loss:
    L_total = MSE + lambda_phys * L_phys   (no BCE term)
...
def train_pinn_model(data_dict, save_dir,
    ...
    config = {
        ...
        'description': (
            ...
            f'inter-layer LayerNorm) + per-appliance heads + L_phys; uniform 10 W threshold'
        ),
    ...
    with open(os.path.join(save_dir, 'pinn_advanced_lnn_redd_dataset_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_advanced_lnn_redd_dataset_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-AdvancedLNN REDD -- Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_advanced_lnn_redd_dataset_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = os.path.join(
        ...
        f"pinn_advanced_lnn_redd_dataset_{timestamp}"
    )
