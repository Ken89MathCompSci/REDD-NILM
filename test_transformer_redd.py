...
def train_transformer_on_appliance(data_dict, appliance_name, window_size=100,
                                    ...
                                    save_dir='models/transformer_redd'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"transformer_redd_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"transformer_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'transformer_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_transformer_on_all_redd_appliances(window_size=100, hidden_size=128, num_layers=3,
                                             num_heads=4, dropout=0.1,
                                             epochs=80, lr=0.001, patience=20):

    data_dict = load_redd_splits()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_save_dir = f"models/transformer_redd_{timestamp}"
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"transformer_redd_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    summary = {
        'timestamp': timestamp, 'dataset': 'REDD', 'model': 'Transformer',
        'window_size': window_size,
...
if __name__ == "__main__":
    ...
    test_transformer_on_all_redd_appliances(
