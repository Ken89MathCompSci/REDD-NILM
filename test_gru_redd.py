...
def train_gru_on_appliance(data_dict, appliance_name, window_size=100,
                            hidden_size=128, num_layers=2, dropout=0.2,
                            bidirectional=True,
                            epochs=80, lr=0.001, patience=20,
                            save_dir='models/gru_redd'):
    ...
    print(f"Starting GRU training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"gru_redd_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"gru_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'gru_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_gru_on_all_redd_appliances(window_size=100, hidden_size=128, num_layers=2,
    ...
    base_save_dir = f"models/gru_redd_{timestamp}"
    ...
    for appliance_name in appliances:
        print(f"\n{'='*60}")
        print(f"Testing GRU on {appliance_name}")
        print(f"{'='*60}\n")
        ...
        try:
            model, history, test_metrics = train_gru_on_appliance(
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"gru_redd_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    summary = {
        'timestamp': timestamp, 'dataset': 'REDD', 'model': 'GRU',
        'window_size': window_size,
    ...
    print(f"\nGRU REDD testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing GRU on REDD dataset...")
    ...
    test_gru_on_all_redd_appliances(
        window_size=100, hidden_size=128, num_layers=2,
        dropout=0.2, bidirectional=True, epochs=80, lr=0.001, patience=20)
